...
class AppStore extends Component {
  ...
  onTabClick = vals => {
    this.setState({activeTabId: vals})
  }
  ...
  clickedList = () => {
    ...
    const activeApps = appsList.filter(items => items.category === activeTabId)
    return activeApps
  }

  render() {
    const {searchText, activeTabId} = this.state
    const selectedList = this.clickedList()
    console.log(selectedList)
    const finalList = selectedList.filter(oitem =>
      oitem.appName.toLowerCase().includes(searchText.toLowerCase()),
    )
          ...
          <ul className="ulItems">
            {tabsList.map(item => (
              <TabItem
                ...
                isActiveT={item.tabId === activeTabId}
                key={item.tabId}
              />
            ))}
          </ul>
          <ul className="applistCont">
            {finalList.map(item => (
              <AppItem appsList={item} key={item.appId} />
            ))}
          </ul>
        </div>
      </div>
